...
Matplotlib需要中文字体：系统 `fonts-noto-cjk`，或用户目录 `~/.local/share/fonts/NotoSansCJKsc-Regular.otf`。本机使用 Google Fonts Noto Sans SC 的报告字形子集 `~/.local/share/fonts/SemidenseChinese.ttf`。完整字体也可从 Noto CJK 官方仓库获取：
