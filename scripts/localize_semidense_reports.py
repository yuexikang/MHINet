...
def curves(run,zh):
    ...
    if rows:
        ...
        def plot(ax,values,label):
            raw,=ax.plot(steps,values,alpha=.22,lw=.6)
            w=min(20,len(values));ax.plot(steps[w-1:],np.convolve(values,np.ones(w)/w,'valid'),label=label,lw=1.2,color=raw.get_color())
...
def main():
    ...
    font=next((p for p in (Path('/usr/share/fonts/opentype/noto/NotoSansCJK-Regular.ttc'),Path.home()/'.local/share/fonts/SemidenseChinese.ttf',Path.home()/'.local/share/fonts/NotoSansCJKsc-Regular.otf') if p.exists()),Path('/missing-chinese-font'))
