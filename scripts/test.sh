#!/usr/bin/env bash
# 真实影像精度评估，必须指定checkpoint；默认完整test，结果不用于调参。
set -euo pipefail
...
if [[ $# -eq 0 || "${1:-}" == --help ]]; then
    ...
    echo '默认GPU_ID=1，完整test（1000对），输出七阶段指标及首个pair的7张图。单元测试：bash scripts/unit_tests.sh'
    exit 0
fi
MHINET_CHECKPOINT="$1"
shift
...
command=("$MHINET_PYTHON" -u -m mhinet.cli evaluate
    ...
    --checkpoint "$MHINET_CHECKPOINT" --split test --visualization-pairs 1
